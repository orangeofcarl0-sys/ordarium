...
| 宿主适配 | **现役叶包**：`@ordarium/host-mcp`（MCP 协议）与 `@ordarium/host-kit`（versioned Host Adapter，供自建宿主对齐宿主合同 + conformance）。**legacy 叶包**：`@ordarium/dsh`（最初的首宿主适配：ToolDefinition/identity/AbortSignal/renderer/注册-dispose、结构化 ContentBlock、`providerPrincipalRef` 瞬态绑定、`recoveryMaterial` 会话绑定——delta-G5-001）**已冻结**，仅为既有集成保留；自 1.3.2 起其公开声明全部带 `@deprecated`（delta-ARCH-003），运行时与形状零变化 |
