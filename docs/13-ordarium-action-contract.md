...
- **增量观测（ORD-BOOT-0，2026-09-11）**：`StateChangeFeed.changes(filter?, cursor?)` 跨主体按**持久账本提交观测序**增量读取已提交修订，补齐"给我 durable position X 之后的 state 修订"这一缺口（`stateHistory` 限单主体、`listStates` 是现值视图，两者都无法给出可持久的全局序）。排序权威是新增的定序元数据表 `ordarium_state_changes`（仅 `(change_seq, namespace, key, revision)` + 外键，无 payload 副本），在 state CAS 的**同一事务**内由存储引擎分配；`written_at` 与隐式 `rowid` 明确不作协调时钟/公开身份。cursor 不透明、全局（与 filter 无关）、到末尾仍返回 resume 位点；畸形 cursor `INVALID_CURSOR` fail closed。能力门 `LedgerCapabilities.stateChangeFeed`（可选）+ `supportsStateChangeFeed`；`OperationLedger` 不增必需成员，既有 ledger 源兼容。交付语义 = `AtLeastOnceObservation + DurableCursor + IdempotentConsumerPossible`。**边界加固（ORD-BOOT-0.1，2026-09-11，patch 1.3.1）**：显式 `limit` 域收紧为 `1..RESOURCE_LIMITS.maxStateChangePageItems`（默认仍 100；`limit=0` 会确定性活锁，直接 `TypeError`）；语法合法的 cursor 若位置 **> 全局 high-water** 亦报 `INVALID_CURSOR`（未来位置绝不静默当作已追平；校验对全局、非 namespace 局部，跨 filter cursor 语义不变）。已知局限：不做 cursor 的数据库身份绑定（`LedgerIdentityBinding = DEFER`），cursor 来自同水位更低的另一库时无法探测。规范全文见 `docs/research/ORD-BOOT-0-state-change-feed-spec.md` 与 `docs/research/ORD-BOOT-0.1-state-change-feed-hardening-spec.md`。
- **操作间依赖边（2026-08-29 决议，处置框架经同日后续决议对齐为 G17）**：内核不提供 operation→operation 的直接边；跨 operation 的依赖、派生与因果关系经管理型 state record 的 refs 表达（state→operation），内核不加第二套边机制——record v2 合同保持零改动。语义缺口的处置由"拒绝写进合同"对齐为**冻结休眠 spec**（`evidence/G17/design-spec.md`："边即管理型 state record" 约定 + `planDependencyCascade` 纯函数助手，触发即按 spec 实施；助手为零合同面纯函数，不构成第二套边机制）。
