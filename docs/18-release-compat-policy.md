...
| 2026-09-11 | RCP-5：`@ordarium/dsh` 叶包冻结为 legacy（文档叙事去 DSH 中心化；自建宿主为一等路径）——登记 `COMPAT-DSH-002`，**尚未随任何版本发布**。下一次发布必须按 §1 ⑤ 披露弃用面（"`@ordarium/dsh` 冻结为 legacy、无新能力、迁移到 core + host-kit/host-mcp"），且①默认值②存储迁移③错误分类④新错误码若无变化明示"无"。本次零 API/schema 变更，故不在 `19` 新增版本行，只记待披露项。 |
