...
> 当前仓库仍是 private `0.2.0` 实现基线。下面描述的是已经冻结的首个公开版本产品表面；现有宽重导出、SQLite v1 record 与 immediate-close lifecycle 会按 `ORDARIUM-GOALS-2` clean break，不构成兼容承诺。
...
当前 private workspace manifest 是 Node.js `>=24.0.0`，当前实现尚会在部分 Node 24 版本显示 `node:sqlite` experimental warning。公开目标把 SQLite 及 DSH durable-default 路径定为 Node.js `>=24.15.0`；core/testing 的最低 Node 将独立验证，不因 SQLite 被无谓抬高。
